...
| 10 | Network egress allowlist | `networks: agent-net` (bridge); no `network_mode: host` | ✅ compose `networks:` block; `grep -F 'network_mode' docker-compose.yaml` → empty |
...
| 14 | Secrets via env vars from `.env`, not baked in | Dockerfile contains zero secrets; compose lists env names only | ✅ `docker history kayaclaw:0.1 --no-trunc \| grep -iE 'TOKEN\|API_KEY'` → empty (only env-var NAMES referenced, never values) |
